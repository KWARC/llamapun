...
pub fn main() {
  ...
  let token_writer = Arc::new(Mutex::new(BufWriter::with_capacity(BUFFER_CAPACITY, token_model_file)));
  let space = ' ';
  let linebreak = '\n';
  ...
  println!("{:?} documents;", corpus_counts.get("document_count").unwrap_or(&0));
  println!("{:?} paragraphs;", corpus_counts.get("paragraph_count").unwrap_or(&0));
  println!(
  ...
  println!("{:?} numeric literals;", corpus_counts.get("num_count").unwrap_or(&0));
  println!("{:?} formulas;", corpus_counts.get("formula_count").unwrap_or(&0));
  println!("{:?} inline cites;", corpus_counts.get("citation_count").unwrap_or(&0));
}
