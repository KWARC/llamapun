...
impl<'dnmrange> DNMRange<'dnmrange> {
  ...
  pub fn get_plaintext(&self) -> &'dnmrange str { &(self.dnm.plaintext)[self.dnm.byte_offsets[self.start]..self.dnm.byte_offsets[self.end]] }
      ...
      None => {
        ...
        if node.is_text_node() {
          ...
          return format!(
            "{}/text()[{}]",
            base,
            get_node_number(&parent, node, &|n: &Node| {
              n.is_text_node()
            }).unwrap()
          );
        } else {
