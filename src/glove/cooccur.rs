...
struct CooccurOptions {
  verbose : bool,
  window_size : u32,  // default context window size
  symmetric : bool, // false: asymmetric, true: symmetric
  memory_limit : u32, // soft limit, in gigabytes, used to estimate optimal array sizes
  max_product : f32, // Cutoff for product of word frequency ranks below which cooccurrence counts will be stored in a compressed full array
  overflow_length : usize // Number of cooccurrence records whose product exceeds max_product to store in memory before writing to disk
}

impl Default for CooccurOptions {
  fn default() -> Self {
    CooccurOptions {
      verbose : true,
      window_size : 15,
...
impl Cooccur {
  ...
  pub fn write_chunk(cr : CRec, length : usize, fout : File) {}


  /// Collect word-word cooccurrence counts from word stream
  pub fn train(&self, input_options : Option<CooccurOptions>) {
    let options = match input_options {
      Some(options) => options,
      None => CooccurOptions::default()
    };
    // let flag, x, y, fidcounter = 1;
  ...
  //     history = malloc(sizeof(long long) * window_size);
    ...
    println_stderr!("COUNTING COOCCURRENCES");
    if options.verbose {
      println_stderr!("window size: {:?}", options.window_size);
      if options.symmetric {
        println_stderr!("context: symmetric");
      } else {
        println_stderr!("context: asymmetric");
      }
      println_stderr!("max product: {:?}", options.max_product);
      println_stderr!("overflow length: {:?}", options.overflow_length);
    }
  }
}
