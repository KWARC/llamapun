...
fn test_string_to_dnm() {
  ...
  if let Ok((doc, dnm)) = dnm_result {
    ...
    assert_eq!(paras.len(), 1, "one auto-wrapped <div class='ltx_para'> paragraph");
...
fn real_ams_para_to_dnm() {
  let contents =
    fs::read_to_string("tests/resources/ams_para_definition.txt").expect("Something went wrong reading the file");
  assert_eq!(
    contents.lines().collect::<Vec<&str>>().len(),
    3,
    "three pre-tokenized sentences in the example"
  );
  assert_eq!(contents.len(), 1263, "we hand-counted the chars in this example.");
  ...
  if let Ok((doc, dnm)) = dnm_result {
    ...
    let ranges: Vec<DNMRange> = simple_tokenizer.sentences(&dnm);
    // dbg!(&ranges);
    assert_eq!(ranges.len(), 1, "one sentence if read as a regular text string");
  }
